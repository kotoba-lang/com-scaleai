(ns scaleai.main
  "Kotodama WASM entrypoint for the Scaleai clean-room actor (L4) — Clojure port.
  ...
  facts (`scaleai.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "scaleai")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"scaleai.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "scaleai-compat" :tier tier :entities entities} 200])
...
(defn start! [] :scaleai-compat/ready)
